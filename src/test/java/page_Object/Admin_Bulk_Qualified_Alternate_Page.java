...
public class Admin_Bulk_Qualified_Alternate_Page extends Admin_BaseClass {
    public Admin_Bulk_Qualified_Alternate_Page(WebDriver driver) {
        this.driver = driver;
    }

    @FindBy(xpath = "//div[text()='Email Management']")
    WebElement emailMngmt;
    @FindBy(xpath = "//*[text()='Bulk Qualified & Alternate']")
    WebElement bulkQualifiedAlternate;
    ...
    public void tableSendCTADisplays() throws Throwable {
        ...
        try {
            ...
            FileUtils.copyFile(screenShot, new File(System.getProperty("user.dir") + "/target/ScreenShots/Bulk Qualified Alternate/TC-1912 Table Send CTA.png"));
            Thread.sleep(1000);
        } catch (Exception e) {
        ...
        if (sendToSelectedRecipients.isDisplayed()) {
            log.info("####### TC-1912 Table Send CTA is displayed and functioning #######");
            logger.log(LogStatus.PASS, "TC-1912 Table Send CTA is displayed and functioning ");
            logger.log(LogStatus.INFO, "Send CTA is displayed and functioning as expected ");
        } else {
            log.info("####### TC-1912 Table Send CTA is displayed and functioning #######");
            logger.log(LogStatus.FAIL, "TC-1912 Table Send CTA is displayed and functioning ");
    ...
    public void tableAreaDisplays() throws Throwable {
        logger = report.startTest("TC-1918 Table Area displays ");
        log.info("####### TC-1918 Table Area displays #######");
        JavascriptExecutor js = (JavascriptExecutor) driver;
        js.executeScript("arguments[0].click();", emailMngmt);
        bulkQualifiedAlternate.click();
        Thread.sleep(1000);
        ...
        snapshot = (TakesScreenshot) driver;
        File screenShot = snapshot.getScreenshotAs(OutputType.FILE);
        FileUtils.copyFile(screenShot, new File(System.getProperty("user.dir") + "/target/ScreenShots/Bulk Qualified Alternate/TC-1918 Table Area display.png"));

        if (sendToSelectedRecipients.isDisplayed()) {
            log.info("####### TC-1918 Table Area displays #######");
            logger.log(LogStatus.PASS, "TC-1918 Table Area displays ");
            logger.log(LogStatus.INFO, "Table Area is displayed and functioning as expected ");
        } else {
            log.info("####### TC-1918 Table Area displays #######");
            logger.log(LogStatus.FAIL, "TC-1918 Table Area displays ");
